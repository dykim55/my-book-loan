...
//				pHtml += " <button class='page_end'>마지막</button>";
	...
	//										html = $.coUtils.textToHtml(html);
		...
		for ( var i = 0; i < trs.length; i++) {
			map[trs[i].getAttribute('rowNo')] = trs[i];
		}
		...
		for ( var i = 0; i < trs.length; i++) {
			if($(".col_check",trs[i]).attr("checked") == "checked"){
				rows[rows.length] = trs[i].getAttribute('rowNo'); 
			}
		}
		return rows;
		...
		trMap ={};
		for ( var i = 0; i < trs.length; i++) {
			trMap[trs[i].getAttribute('rowNo')] = trs[i];
		}
		for ( var i = 0; i < param.length; i++) {
		...
		for ( var i = 0; i < trs.length; i++) {
			...
			if(	sd == t[0].grid.statusData.I 
				...
				redata[row.getAttribute('rowNo')] = rowData;
				saveFlag = false;
			}
		...
		if(param != undefined){
			...
			for ( var i = 0; i < trs.length; i++) {
				pp[i] = trs[i].getAttribute('rowNo');
				
			}
		}
		
		for ( var i = 0; i < trs.length; i++) {
			rows_rowNo[trs[i].getAttribute('rowNo')] = trs[i];
		}
		...
		var trs = t[param];
		if(flag){
			trs.setAttribute("class", "tr_highlight");
			//trs.addClass("tr_highlight");
		}else{
			trs.removeAttr("class");
			//trs.setAttribute("class", "tr_highlight");
			//trs.removeClass("tr_highlight");
		}
		*/
		...
		reRowNos = [];
		for ( var i = 0; i < trs.length; i++) {
			$.coUtils.arrayAdd(reRowNos,trs[i].getAttribute('rowNo'));
		}
		return reRowNos;
	}else if("setCol" == tag){
		t = t[0];
		...
		if(param == undefined){
			...
			for ( var i = 0; i < trs.length; i++) {
				for ( var i = 0; i < param.length; i++) {
					if(trs[i].getAttribute('rowNo') == param[i]){

...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div class='noregist ui-state-default ui-state-active' id='noregist_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		$("<div class='noresult ui-state-default ui-state-active' id='noresult_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		$("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		$("<div class='processing ui-state-default ui-state-active' id='process_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				case "msgbus":
					if (ts.p.url=="") break;
					beginReq();
					channel.send(1, ts.p.url, ts.p.postData, function(resp) {
						if (resp.result) {
							if ($.isFunction(ts.p.beforeProcessing)) {
								if (ts.p.beforeProcessing.call(ts, resp) === false) {
									endReq();
									return;
								}
							}
							addJSONData(resp,ts.grid.bDiv,rcnt,npage>1,adjust);
							$(ts).triggerHandler("jqGridLoadComplete", [resp]);
							if(lc) { lc.call(ts,resp); }
							$(ts).triggerHandler("jqGridAfterLoadComplete", [resp]);
							if (pvis) { ts.grid.populateVisible(); }
							if( ts.p.loadonce || ts.p.treeGrid) {ts.p.datatype = "local";}
							if (npage === 1) { 
								endReq(); 
								if (resp.page == 1 && resp.records == 0) {
									$("#noresult_"+$.jgrid.jqID(ts.p.id)).show();
								} else {
									$("#noresult_"+$.jgrid.jqID(ts.p.id)).hide();
								}
							}
							resp=null;
						} else {
		...
		getOffset = function (iCol) {
			...
			for(i=0;i<=iCol;i++){
				if(ts.p.colModel[i].hidden === false ) {
					...
					ret[0] += ts.grid.headers[i].el.offsetWidth;
				}
			}
		...
		$("th",thr).each(function ( j ) {
			...
			if(ts.p.colModel[j].resizable){
				...
				.dblclick(function(e) {
			    	...
			    	var colName = id.substring((p.id + "_").length);

			    	var testDiv = $('#stringWidth'); // Test DIV for measuring string widths
			        ...
			        var padding = parseInt($($(this).parent()).css('padding-left')) + parseInt($($(this).parent()).css('padding-right')); // Calculate jqGrid header padding
			        testDiv.html($($(this).parent()).text()); // Assign jqGrid header text to test DIV
			        ...
			        $.each($('td[aria-describedby=' + id + ']'), function () // Iterate over all of the rows and figure out which row is the widest.
			        {
			            padding = parseInt($(this).css('padding-left')) + parseInt($(this).css('padding-right')); // Calculate jqGrid cell padding
			            testDiv.html($(this).html()); // Assign jqGrid cell text to test DIV
			        ...
			        $("#rs_m" + $.jgrid.jqID(ts.p.id)).css("display", "none");

			        ts.p.colModel[idx].width = nw;
			        ts.grid.headers[idx].width = nw;
			        ts.grid.headers[idx].el.style.width = nw + "px";
			        ts.grid.cols[idx].style.width = nw + "px";
			        
			        if (ts.grid.footers.length > 0) { ts.grid.footers[idx].style.width = nw + "px"; }

			        if (ts.p.forceFit === true) {
			            nw = ts.grid.headers[idx + ts.p.nv].newWidth || ts.grid.headers[idx + ts.p.nv].width;
			            ts.grid.headers[idx + ts.p.nv].width = nw;
			            ts.grid.headers[idx + ts.p.nv].el.style.width = nw + "px";
			            ts.grid.cols[idx + ts.p.nv].style.width = nw + "px";
			            if (ts.grid.footers.length > 0) { ts.grid.footers[idx + ts.p.nv].style.width = nw + "px"; }
			            ...
			            if (ts.p.footerrow) {
			                ...
			                ts.grid.sDiv.scrollLeft = ts.grid.bDiv.scrollLeft;
			            }
			        }
				});
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						v = $.jgrid.getAccessor(data,nm);
						if (typeof v === 'string') {
							v = v.replace(/</g, "&lt;").replace(/>/g, "&gt;");
						}						
						v = t.formatter( rowid, v, i, data );
